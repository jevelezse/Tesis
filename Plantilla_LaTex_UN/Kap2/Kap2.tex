...
Se realizó una distribución de las variantes según cada técnica sin filtrado (para el caso de omics) para mostrados en las siguientes figuras  \ref{fig:tabla1} y \ref{fig:distribucion}.
...
Una vez obtenidas las regiones se realizo el proceso de hard filtering para el vcf obtenido por el pipe de omics y por el generado por vcftools teniendo los siguientes resultados mostrados por el cuadro \ref{tabla:tabla2}. \\
...
La distribución de las variantes a lo largo de los cromosomas se presenta en la siguiente figura \ref{fig:tabla2}:
...
La representación gráfica de las variantes sobre la distribución a lo largo de los cromosomas figura \ref{fig:variaciones2}.
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.3\textwidth]{Kap2/validacion1}
...
El cuadro \label{tabla:tabla4} muestra la sensibilidad de 96.88\%, una especificidad del 100\% y un PPV de 100.Además después de realizada la limpieza de los datos se hizo la anotación del archivo vcf obtenido y se filtro para el gen CYP2C19 utilizando la versión gráfica de annovar \cite{Yang2015} y obteniéndose el siguiente resultado: \\
