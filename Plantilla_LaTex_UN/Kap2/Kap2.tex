...
La tabla \ref{tabla:tabla3} refleja que para el conjunto de datos no hay falsos negativos ni falsos positivos, pero si falsos positivos, es decir 1033 variantes del conjunto de datos obtenidos que no son reales pero fueron identificadas. (\textit{GATK para calcular estas métricas se compara contra una base de datos que el usuario disponga para determinar variantes existentes}).  La tabla \ref{tabla:tabla4} muestra la sensibilidad, especificidad y el valor predictivo positivo (PPV).
